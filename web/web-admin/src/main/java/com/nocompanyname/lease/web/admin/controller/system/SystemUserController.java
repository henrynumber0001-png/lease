package com.nocompanyname.lease.web.admin.controller.system;
...
public class SystemUserController {
    ...
    public Result<IPage<SystemUserItemVo>> page(@RequestParam long current, @RequestParam long size, SystemUserQueryVo queryVo) {
        IPage<SystemUserItemVo> page = systemUserService.getPage(current, size, queryVo);
    ...
    public Result saveOrUpdate(@RequestBody SystemUser systemUser) {
        systemUserService.saveOrUpdateByUser(systemUser);
        return Result.ok();
    }

    @Operation(summary = "判断后台用户名是否可用")
    @GetMapping("isUserNameAvailable")
    public Result<Boolean> isUsernameAvailabe(@RequestParam String username) {
        boolean isAvailable = systemUserService.isUsernameAvailabe(username);
    ...
    public Result removeById(@RequestParam Long id) {
        systemUserService.removeByUserId(id);
        return Result.ok();
    }
